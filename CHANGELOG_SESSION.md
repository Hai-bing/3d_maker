...
## 19. 2026-08-14：接入 GitHub、开源准备与 3D 超时修复

### 19.1 接入 GitHub
- 初始化 git 仓库，关联远程 `https://github.com/Hai-bing/3d_maker.git`（main 分支），首次推送。
- 新增 `.gitignore`：排除 `.env`、`seed3D_env/`、`static/generated/`、`__pycache__/`、`.idea/` 等，保护 API Key 与本地配置（`.env.example` 模板保留入库）。

### 19.2 开源准备（路径脱敏）
- `start.bat`：ComfyUI 目录改为环境变量 `COMFYUI_DIR`，Flask 目录改用 `%~dp0` 自动定位，移除本机绝对路径。
- `README.md`：`D:\3d_maker\platform` 改为相对路径 `platform`。
- 本文件历史段落中的本机路径 `C:\Users\...` 统一改为 `~/` 通配。
- 新增 MIT `LICENSE`。

### 19.3 启动脚本与配置
- `start.bat`：增加 ComfyUI 目录校验，缺失时快速报错而非死循环。
- `CLAUDE.md`：补充启动地址 `http://localhost:5000`。
- 设置环境变量：`COMFYUI_DIR`（ComfyUI 安装目录）、`HF_ENDPOINT=https://hf-mirror.com`（HuggingFace 国内镜像）。

### 19.4 3D 生成超时修复（显存不足）
- 现象：`Hunyuan3D 生成超时（600 秒）`。
- 根因：RTX 4060 仅 8GB 显存，ComfyUI(SDXL) 与 Hunyuan3D-2mini（权重约 4GB）同时占用导致显存不足。
- 修复：`app.py` 的 `/api/generate-3d` 在调用 Hunyuan3D 前，先请求 ComfyUI `/free` 卸载模型、释放显存。
- 说明：Hunyuan3D 模型缓存在 `~/.cache/hy3dgen/`（非标准 HF 缓存目录），而非 `~/.cache/huggingface/hub/`。
